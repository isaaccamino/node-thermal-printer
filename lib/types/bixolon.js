...
const { scaleImageToWidth, scaleImageToHeight, calculateTwoByteNumber } = require("../utils");
            ...
            if (filetype === "png") {
                ...
                const png = await (async () => {
                    ...
                    if (settings.autoResize) {
                        // TODO: Move these hardcoded values to settings
                        if (pngFromData.height > 60 || pngFromData.width > 160) {
                            const scaleToWidth = scaleImageToWidth({ width: pngFromData.width, height: pngFromData.height }, 160);

                            if (scaleToWidth.height <= 60) {
                                const resizedImageData = await sharp(data).resize(160, null).toBuffer();
                    ...
                    else if (settings.width != null && pngFromData.width > settings.width) {
                        ...
                        return PNG.sync.read(resizedImageData);
                    } else if (settings.height != null && pngFromData.height > settings.height) {
                        const scaleToHeight = scaleImageToHeight({ width: pngFromData.width, height: pngFromData.height }, settings.height);

                        const resizedImageData = await sharp(data).resize(scaleToHeight.width, scaleToHeight.height).toBuffer();
                        return PNG.sync.read(resizedImageData);
                    } else if (settings.width != null && settings.height != null) {
                        const resizedImageData = await sharp(data).resize(settings.width, settings.height).toBuffer();
                        return PNG.sync.read(resizedImageData);
                    }
