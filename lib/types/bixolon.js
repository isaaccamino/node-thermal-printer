...
class Bixolon extends PrinterType {
        ...
        try {
            ...
            if (filetype === "png") {
                const png = await (async () => {
                    ...
                    if (pngFromData.width > 150) {
                        const resizedImageData = await sharp(data).resize(150, null).toBuffer();
                        return PNG.sync.read(resizedImageData);
                    } else if (pngFromData.height > 100) {
                        const resizedImageData = await sharp(data).resize(null, 100, null).toBuffer();
                        return PNG.sync.read(resizedImageData);
                    }
                    return pngFromData;
                })();
    ...
    printImageBuffer(width, height, data, density = 1, dotMatrix = false) {
        ...
        for (let startHeight = 0; startHeight < height; startHeight += 8) {
            ...
            this.append(Buffer.from([0x1b, 0x4a, 0x10]));
