...
const PrinterUtils = require('./utils');
...
class ThermalPrinter {
  constructor(initConfig) {
    ...
    switch (initConfig.type) {
      ...
      case this.types.TANCA:
        ...
        break;
      case this.types.BIXOLON:
        const Bixolon = require('./types/bixolon');
        this.printer = new Bixolon();
        break;
      default:
    ...
    this.config = {
      ...
      breakLine: initConfig.breakLine || BreakLine.NONE,
  ...
  tableCustom(data, printNewLine = true) {
    var cellWidth = this.config.width / data.length;
    var secondLine = [];
  ...
  // ------------------------------------------------ PRINT STRIKETHROUGH ------------------------------------------------
  printStrikethrough = (units) => {
    const UNIT_SIZE = this.printer.characterWidth; // TODO: What determines this?
    if (UNIT_SIZE === undefined) {
      throw new Error('Printer character width is not defined');
    }
  
    // move the print head to the left by `units` (with an offset of half a unit size, so that the dashes will have overlap with the next set of dashes)
    {
      const printerMovementNumber = PrinterUtils.calculateTwoByteNumber((-UNIT_SIZE * units) - (UNIT_SIZE / 2));
      this.add(Buffer.from([0x1b, 0x5c, printerMovementNumber.nL, printerMovementNumber.nH]));
    }
  
    // Print a strike through line (printer head is now to the right by `units` amount, so it should be right in the middle of the last character of the text)
    this.print('-'.repeat(units));
  
    // move the printhead first to the end of the text (half a unit size to the right), then to the left by `units` amount
    {
      const printerMovementNumber = PrinterUtils.calculateTwoByteNumber((UNIT_SIZE / 2) - (UNIT_SIZE * units));
      this.add(Buffer.from([0x1b, 0x5c, printerMovementNumber.nL, printerMovementNumber.nH]));
    }
  
    // Print a strike through line (Second set to remove any gaps between dashes)
    this.print('-'.repeat(units));
  }
  ...
  selectPrintColor(mode) {
    this.append(this.printer.selectPrintColor(mode));
  }
  ...
  // ----------------------------------------------------- LOAD NV IMAGE ----------------------------------------------------
  async loadNVImages(imagePaths) {
    try {
      if (!Array.isArray(imagePaths) || imagePaths.length === 0) {
        throw new Error("No image paths specified for loading into NV memory");
      }
      ...
      imagePaths.forEach(image => {
        ...
        fs.accessSync(image);
        const fileType = image.slice(-4);

        // Check for file type
        if (fileType !== ".png" && fileType !== ".bmp") {
          throw new Error("Image printing supports only PNG or BMP files.");
        }

        // Push information about this file
        imagesInfo.push({
          path: image,
          filetype: fileType
        });
      ...
      try {
        let response = await this.printer.loadNVImages(imagesInfo);
  ...
  printNVImage(num, mode) {
    try {
      if (num < 1 || num > 255) {
        throw new Error("NV image number is outside of range. Please select between 1-255.");
      }
      if (mode !== 0 && mode !== 1 && mode !== 48 && mode !== 49) {
        throw new Error("Mode selection for printing NV image is outside of range. Please select between 0, 1, 48, or 49.");
      }

      let response = this.printer.printNVImage(num, mode);
      this.append(response);
      return response;
    } catch (error) {
      throw error;
  ...
  // ----------------------------------------------------- PRINT IMAGE -----------------------------------------------------
  async printImage(image, settings) {
    try {
      // Check if file exists
      fs.accessSync(image);
      ...
      if (image.slice(-4) === ".png") {
        try {
          let response = await this.printer.printImage(image, "png", settings);
          this.append(response);
          return response;
        } catch (error) {
          throw error;
        }
      } else if (image.slice(-4) === ".bmp") {
        try {
          let response = await this.printer.printImage(image, "bmp", settings);
          this.append(response);
          return response;
        } catch (error) {
          throw error;
